...
size_t
formMessage(unsigned char* str_x_work_msg, unsigned char type, unsigned char* messageBody, size_t messageBodyLen) {
    unsigned char* lenInBytes = intToBytes(messageBodyLen);
    int i = 0;
    str_x_work_msg[i++] = type;
    int k;
    for (k = 0; k < 4; ++k) {
        str_x_work_msg[i++] = lenInBytes[k];
    }
    for (k = 0; k < messageBodyLen; ++k) {
        str_x_work_msg[i++] = messageBody[k];
    }
    free(lenInBytes);
    return (size_t) i;
}
